...
# the Entropic Scree to flawlessy extract the true underlying generative rank.
# ==============================================================================
...
required_packages <- c("Rcpp", "data.table", "infotheo", "ggplot2", "patchwork", "MASS", "stringr")
...
calculate_entropic_scree <- function(data
  ...
  pct_unique_info <- (R_eff / m_valid) * 100
  pct_redundant_info <- (1 - (R_eff / m_valid)) * 100
  total_redundant_volume <- m_valid - R_eff
  ...
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.2f\n", "Total Unique Info Volume (R_eff)", R_eff))
  cat(sprintf(" -> %-50s : %.1f%%\n", "% Total Unique Info", pct_unique_info))
  cat("      (Signal + Structural Uncertainty + Measurement Error)\n")
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.2f\n", "Redundant Signal Info Volume (m - R_eff)", total_redundant_volume))
  cat(sprintf(" -> %-50s : %.1f%%\n", "% Redundant Signal Info", pct_redundant_info))
  cat("-----------------------------------------------------------------\n")
  ...
  cat("=================================================================\n")
  cat(sprintf(" -> %-43s : %d\n", "Automated Extracted Elbow Rank (r)", K_elbow))
    ...
    while (TRUE) {
      ...
      if (tolower(ans) %in% c("y", "yes")) {
        ...
        if (!is.na(parsed_k) && parsed_k > 0 && parsed_k <= m_valid) {
          ...
          signal_info_volume_prev <- R_eff * signal_weight_prev
          total_signal_footprint_prev <- signal_info_volume_prev + total_redundant_volume
          
          AIG_prev <- total_signal_footprint_prev / K_final
          core_eigenvals_prev <- eig_vals[1:K_final]
          p_core_prev <- core_eigenvals_prev / sum(core_eigenvals_prev)
          FSG_prev <- p_core_prev * total_signal_footprint_prev
          
          cat("\n=================================================================\n")
          cat(sprintf(" (PREVIEW) ELBOW LATENT METRICS FOR r = %d\n", K_final))
          cat("=================================================================\n")
          cat(sprintf(" -> Preview Rank (r)            : %d\n", K_final))
          cat(sprintf(" -> Avg Info Gravity (AIG)      : %.2f\n", AIG_prev))
          cat(" -> Factor Specific Gravity (FSG):\n")
          print(round(FSG_prev, 3))
          cat("=================================================================\n\n")
  ...
  signal_info_volume <- R_eff * signal_weight
  total_signal_footprint <- signal_info_volume + total_redundant_volume
  
  AIG <- total_signal_footprint / K_final
  core_eigenvals <- eig_vals[1:K_final]
  p_core <- core_eigenvals / sum(core_eigenvals)
  FSG_final <- p_core * total_signal_footprint
  
  cat("\n=================================================================\n")
  cat(" (FINAL) ELBOW LATENT METRICS (based user-confirmed r)\n")
  cat("=================================================================\n")
  cat(sprintf(" -> Final Retained Rank (r)     : %d\n", K_final))
  cat(sprintf(" -> Avg Info Gravity (AIG)      : %.2f\n", AIG))
  cat(" -> Factor Specific Gravity (FSG):\n")
  print(round(FSG_final, 3))
  cat("=================================================================\n\n")
  ...
  noise_info_volume <- R_eff * noise_weight
  true_redundancy <- m_valid - R_eff
  
  cat("=================================================================\n")
  cat(sprintf(" (FINAL) TRIPARTITE STRUCTURAL COMPOSITION (based user-confirmed r = %d)\n", K_final))
  cat("=================================================================\n")
  cat(sprintf(" -> %-50s : %d\n", "Valid Variables (m)", m_valid))
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.2f\n", "Total Signal Info Volume (Unique + Redundant)", total_signal_footprint))
  cat(sprintf(" -> %-40s : %.3f\n", "   (Unique Signal Info Volume)", signal_info_volume))
  cat(sprintf(" -> %-40s : %.3f\n", "   (Redundant Signal Info Volume)", true_redundancy))
  cat("-----------------------------\n")
  cat(sprintf(" -> %-50s : %.2f\n", "Idiosyncratic Noise Volume", noise_info_volume))
  cat("      (Structural Uncertainty + Measurement Error)\n")
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.2f%%\n", "% Total Signal Info (Unique + Redundant)", (total_signal_footprint / m_valid) * 100))
  cat(sprintf(" -> %-40s : %.3f%%\n", "   (% Unique Signal)", (signal_info_volume / m_valid) * 100))
  cat(sprintf(" -> %-40s : %.3f%%\n", "   (% Redundant Signal)", (true_redundancy / m_valid) * 100))
  cat("-----------------------------\n")
  cat(sprintf(" -> %-50s : %.2f%%\n", "% Idiosyncratic Noise", (noise_info_volume / m_valid) * 100))
  cat("      (Structural Uncertainty + Measurement Error)\n")
  cat("=================================================================\n\n")
  
  # ============================================================================
  # EXTENDED FACTOR SPECIFIC GRAVITY (FSG) CALCULATIONS
  # ============================================================================
  ...
  footprint_bulk <- (R_eff * (sig_var_bulk / m_valid)) + total_redundant_volume
  FSG_extended_bulk <- p_extended_bulk * footprint_bulk
  ...
  footprint_kaiser <- (R_eff * (sig_var_kaiser / m_valid)) + total_redundant_volume
  FSG_extended_kaiser <- p_extended_kaiser * footprint_kaiser
  ...
  return(list(
    ...
    total_signal_footprint = total_signal_footprint,
    signal_info_volume = signal_info_volume,
    noise_info_volume = noise_info_volume,
    true_redundancy = true_redundancy,          
    AIG = AIG,
    FSG_final = FSG_final,
    FSG_extended_bulk = FSG_extended_bulk,
    FSG_extended_kaiser = FSG_extended_kaiser,
...
generate_true_mixed_proxies <- function(s1_continuous, m_proxies, max_interaction_order = 3, max_polynomial_order = 3, int_scaling = 1, continuous_ratio = 1.0) {
  ...
  # --- NEW: STANDARDIZE THE ENTIRE DESIGN MATRIX ---
...
# ==============================================================================
# 3. EXECUTE SIMULATION AND EXTRACTION
# ==============================================================================
set.seed(19862026)
...
cat(sprintf("\n[***] LATENT GENERATIVE RANK (True Factor Copula): %.2f\n\n", true_continuous_ceiling))
...
# Print the Algebraic K_rlzd extracted safely from the function!
cat(sprintf("[***] ALGEBRAIC K_rlzd (Active Configurational Axes): %d\n", true_universe$active_terms))
...
# Calculate PCA eigenvalues (variance explained)
...
# PCA will only return N eigenvalues (5000), padding the rest with exactly 0
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  # Extract K_rlzd (Realized Configurational Rank) and R_alg (Effective Latent Algebraic Rank)
  ...
  # Staggering every label across unique vertical heights absolutely guarantees 
  ...
  p_pca <- ggplot2::ggplot(pca_data, ggplot2::aes(x = Rank, y = Eigenvalue)) +
    ...
    # Add True Rank (r) for visual anchoring
    ggplot2::geom_vline(xintercept = K_TRUE, color = "forestgreen", linetype = "solid", linewidth = 1.2) +
    ggplot2::annotate("text", x = K_TRUE, y = pca_y_90, label = sprintf("True Rank (r = %d)", K_TRUE), hjust = -0.05, color = "forestgreen", fontface = "bold", size = ref_size) +
    
    # Add R_alg (Effective Latent Algebraic Rank) - Placed LEFT of the line
    ...
    # Add K_rlzd (Realized Configurational Rank)
    ...
    ggplot2::annotate("text", x = (N_ROWS - 1), y = pca_y_30, label = "Standard PCA Rank Ceiling (N-1)", hjust = 1.05, size = ref_size) +
  ...
  p_ent <- ggplot2::ggplot(df_compare[df_compare$Method == "Entropic Scree", ], 
    ...
    # Add True Rank (r)
    ggplot2::geom_vline(xintercept = K_TRUE, color = "forestgreen", linetype = "solid", linewidth = 1.2) +
    ggplot2::annotate("text", x = K_TRUE, y = ent_y_90, label = sprintf("True Rank (r = %d)", K_TRUE), hjust = -0.05, color = "forestgreen", fontface = "bold", size = ref_size) +
