...
required_packages <- c("Rcpp", "data.table", "infotheo", "ggplot2",
"patchwork", "MASS", "stringr", "pkgbuild")
...
if(length(missing_packages) > 0) {
  cat("Installing missing dependencies:", paste(missing_packages,
collapse = ", "), "\n")
...
if (.Platform$OS.type == "windows") {
  # suppressWarnings hides the native pkgbuild warning so our clean
custom block prints alone
  if (!suppressWarnings(pkgbuild::has_rtools())) {
    stop(
      ...
      " You must install the version of Rtools that matches this R
version.\n\n",
...
NumericMatrix fast_parallel_MI(IntegerMatrix mat, int num_bins, int cores) {
    ...
    // Pre-calculate margins and single entropies
    for(int j = 0; j < p; ++j) {
        for(int i = 0; i < n; ++i) {
            int val = mat(i, j) - 1;
        ...
        for(int i = 0; i < p; ++i) {
            for(int j = i + 1; j < p; ++j) {
                ...
                for(int row = 0; row < n; ++row) {
                    ...
                    if(val1 >= 0 && val1 < num_bins && val2 >= 0 &&
val2 < num_bins) {
...
calculate_entropic_scree <- function(data
  ...
  # ----------------------------------------------------------------------------
  if (ncol(dt) < 2) {
    stop("Execution Halted: The input dataset must contain at least 2
columns to calculate mutual information.")
  }
        ...
        if (nrow(num_mat) > 0) {
          ...
          if (qr_decomp$rank < ncol(qr_mat)) {
            ...
            if (length(lin_combos) > 0) {
              cat(sprintf("      -> Purged %d perfectly collinear
variables to protect downstream SIA.\n", length(lin_combos)))
              dt[, (lin_combos) := NULL]
            }
          }
        }
      } else {
        cat("      -> Matrix exceeds absolute LINPACK bounds even when
sub-sampled. Bypassing...\n")
      }
    }
  } else {
  ...
  cat(sprintf("[6/10] Computing %d x %d Mutual Information Matrix (C++
OpenMP)...\n", p, p))
  ...
  MI_mat <- fast_parallel_MI(mat_data, num_bins = num_bins, cores =
safe_target_cores)
  ...
  cat("[10/10] Calculating Total Unique Probabilistic Volume (R_eff)
and Estimating Phase Transitions...\n")
  ...
  # ==========================================================================
  # STEP 1: MACRO GAP BOUNDARY (IDIOSYNCRATIC INFORMATIONAL VARIANCE
CLIFF DETECTION)
  # ==========================================================================
  ...
  if (length(valid_search_space) > 10) {
    ...
    noise_start_idx <- min(valid_k + max(3, floor(n_active * 0.05)),
n_active - 5)
    ...
    if(length(noise_tail_idx) > 0) {
      ...
      if (length(macroscopic_gap_indices) > 0) {
        ...
        if (max_noise_gap > 1e-9) macro_gap_ratio <- macro_actual_gap
/ max_noise_gap
      }
    }
  }

  search_start_idx <- if (!is.na(top_of_bulk_idx)) max(2,
top_of_bulk_idx - 1) else valid_k
  ...
  if (n_valid_search >= 3) {
    ...
    if (!is.na(top_of_bulk_idx) && top_of_bulk_idx < 4) {
      ...
      # Standard Evaluation
      search_limit <- if (!is.na(top_of_bulk_idx)) max(2,
top_of_bulk_idx - 1) else length(all_log_gaps)
      if (search_limit >= 2) {
  ...
  if (search_start_idx >= 3) {
    ...
    if (!is.na(top_of_bulk_idx) && !is.na(macro_actual_gap)) {
      ...
      stitched_eig_vals[top_of_bulk_idx:length(stitched_eig_vals)] <-
stitched_eig_vals[top_of_bulk_idx:length(stitched_eig_vals)] +
stitch_constant
      stitch_applied <- TRUE
    }

    # 2. Dynamically scale sigma using t-dist to target a family-wise
false positive rate of 1/100
    ...
    for (i in seq(search_start_idx, 1, by = -1)) {
      ...
      if (ref_start > length(stitched_eig_vals)) next # Safety check
at the absolute tail
      ...
      if (length(ref_vals) == triple_tap_window) {
        ...
        # Fallback for the absolute tail where full window points are
not available. df = n - 1
  ...
  # Safely capture the multiplier for the printout if it never broke
but we had space
  if (is.na(triple_tap_multiplier) && search_start_idx >= 3) {
    triple_tap_multiplier <- max(2.0, qt(1 - (1 / (100 *
search_start_idx)), df = triple_tap_window - 2))
  }
  ...
  if (requireNamespace("ggplot2", quietly = TRUE)) {
    ...
    if (K_log_gap == K_triple_tap) {
      lines_geom <- ggplot2::geom_vline(xintercept = K_log_gap, color
= "forestgreen", linetype = "dashed", linewidth = 1.2)
      sub_title_text <- sprintf("Converged: Observed Generative Rank &
Extended Signal Tail = %d", K_log_gap)
    } else {
      lines_geom <- list(
        ggplot2::geom_vline(xintercept = K_log_gap, color = "#D55E00",
linetype = "dashed", linewidth = 1.2),
        ggplot2::geom_vline(xintercept = K_triple_tap, color =
"purple", linetype = "twodash", linewidth = 1.2)
      )
      sub_title_text <- sprintf("Observed Generative Rank (Orange) =
%d | Extended Signal Tail (Purple) = %d", K_log_gap, K_triple_tap)
    }
    ...
    plot_df_zoom <- data.frame(Rank = zoom_start:zoom_end, Eigenvalue
= eig_vals[zoom_start:zoom_end])

    p_scree_zoom <- ggplot2::ggplot(plot_df_zoom, ggplot2::aes(x =
Rank, y = Eigenvalue)) +
...
unique(floor(pretty(seq(min(x), max(x)))))) +
      ggplot2::labs(title = "Zoomed View", x = "Eigenvalue Index", y =
"Log(Eigenvalue)") +
...
hjust = 0.5, size = 12))
    ...
    plot_df_macro <- data.frame(Rank = 1:macro_end, Eigenvalue =
eig_vals[1:macro_end])
    macro_y_max <- if(length(eig_vals) >= 2) eig_vals[2] * 1.1 else
max(eig_vals)

    p_scree_macro <- ggplot2::ggplot(plot_df_macro, ggplot2::aes(x =
Rank, y = Eigenvalue)) +
      ggplot2::geom_line(color = "dodgerblue", linewidth = 1) +
      ggplot2::geom_point(color = "dodgerblue", size = 2) +
      lines_geom +
      ...
      ggplot2::labs(title = "Macro View", x = "Eigenvalue Index", y =
"Log(Eigenvalue)") +
      ggplot2::theme_minimal(base_size = 14) +
      ggplot2::theme(plot.title = ggplot2::element_text(face = "bold",
hjust = 0.5, size = 12))
...
ggplot2::element_text(face = "bold", size = 16, hjust = 0.5),
                                 plot.subtitle =
ggplot2::element_text(size = 14, hjust = 0.5))
        )
      print(combined_plot)
    } else {
      print(p_scree_macro)
  ...
  cat(sprintf(" -> %-50s : %.3f%%\n", "Synergistic Curvature Deficit
Ratio (SCDR)", SCDR))
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.3f\n", "Total Unique Probabilistic Volume
(R_eff)", R_eff))
  cat(sprintf(" -> %-50s : %.3f%%\n", "%", pct_prob_volume))
  cat("      (Unique Signal Volume + Structural Uncertainty\n")
  cat("      + Independent Measurement Error + Unshared Signal Geometry)\n")
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.3f\n", "Redundant Signal Volume (m -
R_eff)", redundant_signal_volume))
...
Idiosyncratic", top_of_bulk_idx))
    cat("    Informational Variance Bulk\n")
    cat(sprintf(" -> %-43s : %.6f\n", "Macro Gap Baseline (Max Bulk
Gap)", macro_max_bulk_gap))
    cat(sprintf(" -> %-43s : %.2fx Baseline\n", "Actual Macro Gap
Magnitude", macro_gap_ratio))
  } else {
    cat(sprintf(" -> %-43s : %s\n", "Identified Top of Idiosyncratic
Informational Variance Bulk", "Failed (Defaulted to Kaiser)"))
  }
  cat("-----------------------------------------------------------------\n")
  cat(" [Engine A: Log-Gap]\n")
  cat(sprintf(" -> %-43s : %d%s\n", "Observed Generative Rank
(K_roots)", K_log_gap, ifelse(log_gap_fallback, " [Forced by Macro
Boundary]", "")))
  if (!is.na(log_gap_ratio)) {
    cat(sprintf(" -> %-43s : %.2fx (%.1f%% Drop)\n", "Log-Gap
Magnitude", log_gap_ratio, log_gap_pct_drop))
  }
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" [Engine B: Triple-Tap (%d-Point Linear)]\n", triple_tap_window))
  cat(sprintf(" -> %-43s : %d%s\n", "Extended Signal Tail Rank
(K_extended)", K_triple_tap, ifelse(triple_tap_fallback, " [Failed to
trigger; defaulted to Log-Gap]", "")))
  cat(sprintf(" -> %-43s : %s\n", "Macro-Stitch Applied",
ifelse(stitch_applied, "Yes (+0.5x Macro Gap to Bulk)", "No")))
  if (!is.na(triple_tap_expected_val)) {
    cat(sprintf(" -> %-43s : %.5f\n", "Expected Local Eigenvalue",
triple_tap_expected_val))
    cat(sprintf(" -> %-43s : %.5f\n", "Actual Local Eigenvalue",
triple_tap_actual_val))
    cat(sprintf(" -> %-43s : %.6f\n", "Calculated Local Baseline Sigma
(SD)", triple_tap_local_sd))
  }
  if (!is.na(triple_tap_multiplier)) {
    cat(sprintf(" -> %-43s : %.2f (1-α = %.6f)\n", "Required Local
t-Multiplier", triple_tap_multiplier, prob_target))
  }
  if (!is.na(triple_tap_actual_sigma)) {
    cat(sprintf(" -> %-43s : %.2f Sigma Breakout\n", "Actual vs
Expected", triple_tap_actual_sigma))
  }
  cat("===================================================================================\n")
  ...
  if (interactive_mode) {
    cat("\n[WARNING]: The automated extractors rely on statistical
heuristics and\n")
    cat("may not perfectly align with the true structural elbow of
your specific dataset.\n")
    cat("Please visually examine the generated scree plot.\n\n")

    while (TRUE) {
      # 1. Prompt for Observed Generative Rank
      ans_r <- trimws(readline(prompt = sprintf("Enter the Observed
Generative Rank (K_roots) [Press Enter to keep %d]: ", K_roots)))
      if (ans_r != "") {
        parsed_r <- suppressWarnings(as.integer(ans_r))
        if (!is.na(parsed_r) && parsed_r > 0 && parsed_r <= m_valid)
K_roots <- parsed_r
      }

      # 2. Prompt for Extended Signal Tail Rank
      ans_ext <- trimws(readline(prompt = sprintf("Enter the rank of
the Extended Signal Tail [Press Enter to keep %d]: ", K_extended)))
      if (ans_ext != "") {
        parsed_ext <- suppressWarnings(as.integer(ans_ext))
        if (!is.na(parsed_ext) && parsed_ext > 0 && parsed_ext <=
m_valid) K_extended <- parsed_ext
      }

      if (K_roots > K_extended) {
        cat("[-] Warning: The Extended Signal Tail Rank should be >=
the Observed Generative Rank.\n")
      }
      ...
      if (requireNamespace("ggplot2", quietly = TRUE)) {
        ...
        if (K_log_gap == K_triple_tap) {
          prev_lines <- ggplot2::geom_vline(xintercept = K_log_gap,
color = "gray60", linetype = "dashed", linewidth = 1)
        } else {
          prev_lines <- list(
            ggplot2::geom_vline(xintercept = K_log_gap, color =
"gray60", linetype = "dashed", linewidth = 1),
            ggplot2::geom_vline(xintercept = K_triple_tap, color =
"gray70", linetype = "twodash", linewidth = 1)
          )
        }

        # Draw user lines: Solid for roots, dashed for extended tail
        root_line <- ggplot2::geom_vline(xintercept = K_roots, color =
"forestgreen", linetype = "solid", linewidth = 1.2)
        if (K_roots != K_extended) {
          ext_line <- ggplot2::geom_vline(xintercept = K_extended,
color = "forestgreen", linetype = "dashed", linewidth = 1.2)
        } else {
          ext_line <- NULL
        }
        ...
        zoom_end_upd <- min(length(eig_vals), max(K_log_gap,
K_triple_tap, K_extended) + 15)

        plot_df_zoom_upd <- data.frame(Rank =
zoom_start_upd:zoom_end_upd, Eigenvalue =
eig_vals[zoom_start_upd:zoom_end_upd])

        p_scree_zoom_upd <- ggplot2::ggplot(plot_df_zoom_upd,
ggplot2::aes(x = Rank, y = Eigenvalue)) +
          ...
          ggplot2::scale_x_continuous(breaks = function(x)
unique(floor(pretty(seq(min(x), max(x)))))) +
          ggplot2::labs(title = "Zoomed View", x = "Eigenvalue Index
(m)", y = "Log(Eigenvalue)") +
          ggplot2::theme_minimal(base_size = 14) +
          ggplot2::theme(plot.title = ggplot2::element_text(face =
"bold", hjust = 0.5, size = 12))
        ...
        plot_df_macro_upd <- data.frame(Rank = 1:macro_end_upd,
Eigenvalue = eig_vals[1:macro_end_upd])
        macro_y_max <- if(length(eig_vals) >= 2) eig_vals[2] * 1.1
else max(eig_vals)

        p_scree_macro_upd <- ggplot2::ggplot(plot_df_macro_upd,
ggplot2::aes(x = Rank, y = Eigenvalue)) +
          ggplot2::geom_line(color = "dodgerblue", linewidth = 1) +
          ggplot2::geom_point(color = "dodgerblue", size = 2) +
          prev_lines + final_lines +
          ggplot2::scale_y_continuous(trans = 'log10') +
          ggplot2::coord_cartesian(ylim = c(NA, macro_y_max)) +
          ggplot2::labs(title = "Macro View", x = "Eigenvalue Index
(m)", y = "Log(Eigenvalue)") +
          ggplot2::theme_minimal(base_size = 14) +
          ggplot2::theme(plot.title = ggplot2::element_text(face =
"bold", hjust = 0.5, size = 12))
        ...
        if (requireNamespace("patchwork", quietly = TRUE)) {
          combined_plot_upd <- (p_scree_macro_upd + p_scree_zoom_upd) +
            patchwork::plot_annotation(
              title = "Entropic Scree Results",
              subtitle = sprintf("User Confirmed: Observed Generative
Rank (Solid) = %d | Extended Signal Tail (Dashed) = %d", K_roots,
K_extended),
              theme = ggplot2::theme(plot.title =
ggplot2::element_text(face = "bold", size = 16, hjust = 0.5),
                                     plot.subtitle =
ggplot2::element_text(size = 14, hjust = 0.5))
      ...
      total_signal_volume_prev <- unique_signal_volume_prev +
redundant_signal_volume
      ...
      cat("\n===================================================================================\n")
      cat(sprintf(" (PREVIEW) ELBOW METRICS (K_roots = %d, K_extended
= %d) [K_extended Volume Rebundled to K_roots]\n", K_roots,
K_extended))
      cat("===================================================================================\n")
      STP_prev <- FSIG_prev / FSIG_prev[1]
      cat(sprintf(" -> Average Informational Gravity (AIG): %.3f
Variable Equivalents\n", AIG_prev))
      ...
      ans_confirm <- trimws(readline(prompt = "Do you want to finalize
these ranks? (Y/N): "))
  ...
  # 1. Capture Total Shared Signal Volume strictly up to the Extended
Signal Tail
  ...
  # 2. Rebundle AIG and FSIG strictly into the Observed Generative
Rank (K_roots)
  ...
  cat("-----------------------------------------------------------------\n")
  cat(" [Informational Gravity Metrics Estimates] (K_extended Volume
Rebundled to K_roots)\n")
  cat(sprintf(" -> Average Informational Gravity (AIG): %.3f Variable
Equivalents\n", AIG))
  cat(" -> Factor-Specific Informational Gravity (FSIG):\n")
  ...
  # ============================================================================
  idiosyncratic_variance <- if (K_extended < m_valid)
sum(eig_vals[(K_extended + 1):m_valid]) else 0
  idiosyncratic_weight <- idiosyncratic_variance / m_plus
  idiosyncratic_volume <- R_eff * idiosyncratic_weight

  cat("===================================================================================\n")
  cat(sprintf(" (FINAL) TRIPARTITE STRUCTURAL COMPOSITION (Rebundling
Based on K_extended = %d)\n", K_extended))
  cat("===================================================================================\n")
  cat(sprintf(" -> %-50s : %d\n", "Valid Variables (m)", m_valid))
  cat(sprintf(" -> %-50s : %.2f\n", "Centered Trace (Tr_Mc)", Tr_Mc))
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.3f\n", "Total Shared Signal Volume
(Unique + Redundant)", total_signal_volume))
  cat(sprintf(" -> %-40s : %.3f\n", "   (Unique Signal Volume)",
unique_signal_volume))
  cat(sprintf(" -> %-40s : %.3f\n", "   (Redundant Signal Volume)",
redundant_signal_volume))
  cat("-----------------------------\n")
  cat(sprintf(" -> %-50s : %.3f\n", "Idiosyncratic Informational
Variance", idiosyncratic_volume))
  cat("      (Structural Uncertainty + Independent Measurement Error\n")
  cat("      + Unshared Signal Geometry)\n")
  cat("-----------------------------------------------------------------\n")
  cat(sprintf(" -> %-50s : %.3f%%\n", "% Total Shared Signal",
(total_signal_volume / m_valid) * 100))
  cat(sprintf(" -> %-40s : %.3f%%\n", "   (% Unique Signal)",
(unique_signal_volume / m_valid) * 100))
  cat(sprintf(" -> %-40s : %.3f%%\n", "   (% Redundant Signal)",
(redundant_signal_volume / m_valid) * 100))
  cat("-----------------------------\n")
  cat(sprintf(" -> %-50s : %.3f%%\n", "% Idiosyncratic Informational
Variance", (idiosyncratic_volume / m_valid) * 100))
  cat("      (Structural Uncertainty + Independent Measurement Error\n")
  cat("      + Unshared Signal Geometry)\n")
  cat("===================================================================================\n\n")
  ...
  total_sig_vol_bulk <- (R_eff * (sig_var_bulk / m_plus)) +
redundant_signal_volume
  ...
  total_sig_vol_kaiser <- (R_eff * (sig_var_kaiser / m_plus)) +
redundant_signal_volume
  # Rebundle gravity strictly into the Observed Generative Rank (K_roots):
...
max_interaction_order = 3, max_polynomial_order = 3, int_scaling = 1,
continuous_ratio = 1.0, MIN_SPARSITY_RATIO = 0.01, MAX_SPARSITY_RATIO
= 0.10, MIN_ROOT_SPARSITY_RATIO = 0.10, MAX_ROOT_SPARSITY_RATIO =
0.50, structural_snr = 5.0, pure_static_ratio = 0.20, pure_root_ratio
= 0.10) {
  ...
  formula_str <- if(eff_cross_order == 1) as.formula("~ .") else
as.formula(paste0("~ .^", eff_cross_order))

  cat(sprintf("      -> Expanding continuous roots up to interaction
order %d and polynomial order %d...\n", eff_cross_order,
max_polynomial_order))
  ...
  if (max_polynomial_order > 1) {
    ...
    if (length(power_list) > 0) {
      ...
      design_mat <- cbind(design_mat,
as.matrix(as.data.frame(power_list, check.names = FALSE)))
    }
  }
  ...
  for(j in 1:m_proxies) {
    # Main effects
    if (n_main > 0) coeffs[is_main, j] <- rnorm(n_main, mean = 0, sd =
term_sds[is_main])
    # Interactions and Polynomials
    if (n_int > 0)  coeffs[is_int, j]  <- rnorm(n_int, mean = 0, sd =
term_sds[is_int] * int_scaling)
  }
  ...
  max_roots_per_proxy <- max(min_roots_per_proxy, round(k *
MAX_ROOT_SPARSITY_RATIO))

  cat(sprintf("      -> Injecting %d Pure Static Proxies (%.1f%% of
total dataset)...\n", m_static, pure_static_ratio * 100))
  cat(sprintf("      -> Injecting %d Pure Root Proxies (%.1f%% of
total dataset, 1 main effect only)...\n", m_pure_root, pure_root_ratio
* 100))
  cat(sprintf("      -> Generating %d Entangled Mixed Proxies...\n", m_mixed))
  cat(sprintf("         * Activating %d to %d Base Roots per proxy
(%.0f%% to %.0f%% Root Sparsity)\n", min_roots_per_proxy,
max_roots_per_proxy, MIN_ROOT_SPARSITY_RATIO * 100,
MAX_ROOT_SPARSITY_RATIO * 100))
  cat(sprintf("         * Randomly sampling %.0f%% to %.0f%% of ALL
aligned terms (Main & Higher-Order).\n", MIN_SPARSITY_RATIO * 100,
MAX_SPARSITY_RATIO * 100))
  ...
  if (m_mixed > 0) {
    ...
    for (j in start_col:end_col) {
      ...
      # Step C & D: Identify ALL aligned terms (both main effects and
higher-order interactions)
      aligned_term_indices <- which(sapply(term_dependencies, function(deps) {
        all(deps %in% selected_roots)
      }))

      # Step E: Sample a random amount BETWEEN Min% and Max% of ALL
aligned terms
      if (length(aligned_term_indices) > 0) {
        # Calculate the absolute floors and ceilings
        absolute_min_to_sample <- max(1,
round(length(aligned_term_indices) * MIN_SPARSITY_RATIO))
        absolute_max_to_sample <- max(absolute_min_to_sample,
round(length(aligned_term_indices) * MAX_SPARSITY_RATIO))

        # Randomly pick how many terms to actually sample (protecting
against min == max)
        if (absolute_min_to_sample == absolute_max_to_sample) {
          n_to_sample <- absolute_min_to_sample
        } else {
          n_to_sample <-
sample(absolute_min_to_sample:absolute_max_to_sample, 1)
        }
  ...
  # Randomly shuffle the columns so the pure static and pure root
variables are randomly dispersed
  ...
  if (active_terms > 0) {
    ...
    active_weights <- ifelse(active_orders == 1,
term_sds[active_indices], term_sds[active_indices] * int_scaling)
    ...
    p_expanded <- expanded_eigen[expanded_eigen > 1e-9] /
sum(expanded_eigen[expanded_eigen > 1e-9])
  ...
  cat(sprintf("      -> Setting Base Rates and Injecting Structural
Uncertainty (Round 1 Noise, SNR = %.2f)...\n", structural_snr))
  ...
  for (j in 1:m_proxies) {
    # Step A: Standardize the deterministic core (Forces pure signal
variance to exactly 1.0)
    ...
    # Step B: Set the Threshold PRIOR to Structural Noise (For Binary
Variables Only)
  ...
  if (m_bin > 0) {
    ...
    apply_copula_mapping <- function(scores) {
      ...
      if (sd(scores) < 1e-9) return(rep(runif(1, min = 0.50, max =
0.95), length(scores)))

      # Because the threshold shift and structural noise are already
permanently baked
      # into the scores, we NO LONGER scale() them here. We simply map
them against
      # the theoretical baseline distribution N(0, total_sd) to get
the final probability.
    ...
    true_bin <- matrix(rbinom(length(prob_mat), 1, prob_mat), nrow =
n, ncol = m_bin)
...
apply_measurement_error <- function(true_universe, snr_continuous =
2.0, binary_error_rate = 0.15) {
  cat(sprintf("      -> Applying Measurement Error (Round 2 Noise:
Continuous SNR = %.2f, Binary Bit-Flip Rate = %.3f)...\n",
...
CONTINUOUS_RATIO <- 0.80 # SET TO 1.00 FOR PURE CONTINUOUS, 0.00 FOR
PURE BINARY, OR ANYWHERE IN BETWEEN
...
MIN_SPARSITY_RATIO <- 0.10      # Minimum % of aligned higher-order
terms a proxy will sample
MAX_SPARSITY_RATIO <- 0.10      # Maximum % of aligned higher-order
terms a proxy will sample
PURE_ROOT_RATIO <- 0.00         # Exactly X% of proxies will purely
measure 1 main effect (Anchor Proxies)
PURE_STATIC_RATIO <- 0.05       # Exactly X% of proxies will be pure,
completely idiosyncratic noise

# IDIOSYNCRATIC NOISE KNOBS
STRUCTURAL_SNR <- 10      # Structural Uncertainty: Determines how
perfectly the proxy reflects the structural manifold (Idiosyncratic
variance)
CONTINUOUS_SNR <- 2       # Pure continuous measurement error (Sensor
degradation)
BINARY_ERROR_RATE <- 0.15 # Pure binary measurement error (Bit-flip)

cat(sprintf("Generating Mixed Synthetic Universe: %s rows, %d Proxies,
%d Latent Drivers...\n", format(N_ROWS, big.mark=","), M_PROXIES,
K_TRUE))
...
cat(sprintf(" -> Generated Root Correlation Matrix (Avg Pairwise |r| =
%.3f)\n", avg_cor))
...
cat(sprintf(" -> Effective Intrinsic Generative Rank (r_eff)
compressed to: %.2f\n", effective_r))
# ---------------------------------------------------
...
observed_data <- apply_measurement_error(true_universe, snr_continuous
= CONTINUOUS_SNR, binary_error_rate = BINARY_ERROR_RATE)

cat("\nDataset is ready. Starting pipeline...\n\n")

# Print Latent Configurational Ranks
cat(sprintf("\n[***] r  (Intrinsic Generative Rank): %d\n\n", K_TRUE))
cat(sprintf("\n[***] r_eff  (Effective Intrinsic Generative Rank):
%.2f\n", effective_r))
cat(sprintf("[***] K_rlzd (Realized Intrinsic Root Expansion Rank)
  : %d\n\n", true_universe$active_terms))
cat(sprintf("[***] K_eff (Effective Intrinsic Root Expansion Rank)
 : %.2f\n", true_universe$K_eff))
...
pca_time <- round(as.numeric(difftime(Sys.time(), start_pca, units =
"secs")), 2)
...
spearman_time <- round(as.numeric(difftime(Sys.time(), start_spearman,
units = "secs")), 2)
...
kpca_time <- round(as.numeric(difftime(Sys.time(), start_kpca, units =
"secs")), 2)
cat(sprintf("Kernel PCA (RBF) completed in %.2f seconds.\n", kpca_time))
rm(K_c); gc(verbose = FALSE)

# --- 5D. BUILD COMPARISON DATAFRAME ---
# Helper function to safely pad (if m > N) or truncate (if N > m) to
match m_total
...
df_compare <- data.frame(
  ...
  Method = factor(rep(c("Standard PCA", "Spearman PCA", "Kernel PCA",
"Entropic Scree"), each = m_total),
                  levels = c("Standard PCA", "Spearman PCA", "Kernel
PCA", "Entropic Scree"))
)
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  pca_y_max <- if(length(pca_eigenvalues) >= 2) pca_eigenvalues[2] *
1.1 else max(pca_eigenvalues)
  spear_y_max <- if(length(spearman_eigenvalues) >= 2)
spearman_eigenvalues[2] * 1.1 else max(spearman_eigenvalues)
  kpca_y_max <- if(length(kpca_eigenvalues) >= 2) kpca_eigenvalues[2]
* 1.1 else max(kpca_eigenvalues)
  ent_y_max <- if(length(results$eigenvalues) >= 2)
results$eigenvalues[2] * 1.1 else max(results$eigenvalues)
  ...
  get_heights <- function(y_min, y_max) {
    list(
      ...
      h52 = y_min + (y_max - y_min) * 0.525, # Midpoint between K_rlzd
(h75) and Rank Ceiling (h30)
  ...
  ent_data <- df_compare[df_compare$Method == "Entropic Scree" &
df_compare$Rank < M_PROXIES, ]

  # ---------------------------------------------------------
  # PANEL 1: STANDARD PCA
  # ---------------------------------------------------------
  p_pca <- ggplot2::ggplot(pca_data, ggplot2::aes(x = Rank, y = Eigenvalue)) +
    ggplot2::geom_line(data = pca_data[pca_data$Rank < n_rows, ],
color = "firebrick", linewidth = 1) +
    ggplot2::geom_line(data = pca_data[pca_data$Rank >= (n_rows - 1),
], color = "gray60", linetype = "dashed", linewidth = 1) +
    ggplot2::geom_vline(xintercept = K_TRUE, color = "forestgreen",
linetype = "solid", linewidth = 1.2) +
    ggplot2::annotate("text", x = K_TRUE, y = hpca$h90, label =
sprintf(" r \n (%d)", K_TRUE), hjust = -0.1, color = "forestgreen",
fontface = "bold", size = ref_size) +
    ggplot2::geom_vline(xintercept = K_rlzd, color = "purple",
linetype = "dotdash", linewidth = 1) +
    ggplot2::annotate("text", x = K_rlzd, y = hpca$h75, label =
sprintf(" K_rlzd \n (%d)", K_rlzd), hjust = 1.05, color = "purple",
fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = pca_kaiser, color = "darkorange",
linetype = "dashed", linewidth = 1) +
    ggplot2::annotate("text", x = pca_kaiser, y = hpca$h52, label =
sprintf("Kaiser, E>1 \n(%d) ", pca_kaiser), hjust = 1.05, color =
"darkorange", fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = (n_rows - 1), color = "black",
linetype = "dotted", linewidth = 1) +
    ggplot2::annotate("text", x = (n_rows - 1), y = hpca$h30, label =
"Rank \nCeiling (N-1)", hjust = 1.05, size = ref_size) +
    ...
    ggplot2::labs(title = "Standard PCA", x = "Eigenvalue Index [Log
Scale]", y = "Eigenvalue") +
...
= ggplot2::element_text(face="bold"))
  ...
  # ---------------------------------------------------------
  p_spear <- ggplot2::ggplot(spear_data, ggplot2::aes(x = Rank, y =
Eigenvalue)) +
    ggplot2::geom_line(data = spear_data[spear_data$Rank < n_rows, ],
color = "darkgoldenrod", linewidth = 1) +
    ggplot2::geom_line(data = spear_data[spear_data$Rank >= (n_rows -
1), ], color = "gray60", linetype = "dashed", linewidth = 1) +
    ggplot2::geom_vline(xintercept = K_TRUE, color = "forestgreen",
linetype = "solid", linewidth = 1.2) +
    ggplot2::annotate("text", x = K_TRUE, y = hspear$h90, label =
sprintf(" r \n (%d)", K_TRUE), hjust = -0.1, color = "forestgreen",
fontface = "bold", size = ref_size) +
    ggplot2::geom_vline(xintercept = K_rlzd, color = "purple",
linetype = "dotdash", linewidth = 1) +
    ggplot2::annotate("text", x = K_rlzd, y = hspear$h75, label =
sprintf(" K_rlzd \n (%d)", K_rlzd), hjust = 1.05, color = "purple",
fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = spearman_kaiser, color =
"darkorange", linetype = "dashed", linewidth = 1) +
    ggplot2::annotate("text", x = spearman_kaiser, y = hspear$h52,
label = sprintf("Kaiser, E>1 \n(%d) ", spearman_kaiser), hjust = 1.05,
color = "darkorange", fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = (n_rows - 1), color = "black",
linetype = "dotted", linewidth = 1) +
    ggplot2::annotate("text", x = (n_rows - 1), y = hspear$h30, label
= "Rank \nCeiling (N-1)", hjust = 1.05, size = ref_size) +
    ggplot2::scale_x_log10(breaks = log10_breaks) +
    ggplot2::coord_cartesian(ylim = c(pca_y_min, spear_y_max)) +
    ggplot2::labs(title = "Spearman Rank PCA", x = "Eigenvalue Index
[Log Scale]", y = "Eigenvalue") +
    ggplot2::theme_minimal(base_size = 12) + ggplot2::theme(plot.title
= ggplot2::element_text(face="bold"))
  ...
  p_kpca <- ggplot2::ggplot(kpca_data, ggplot2::aes(x = Rank, y = Eigenvalue)) +
    ggplot2::geom_line(data = kpca_data[kpca_data$Rank < n_rows, ],
color = "mediumorchid4", linewidth = 1) +
    ggplot2::geom_line(data = kpca_data[kpca_data$Rank >= (n_rows -
1), ], color = "gray60", linetype = "dashed", linewidth = 1) +
    ggplot2::geom_vline(xintercept = K_TRUE, color = "forestgreen",
linetype = "solid", linewidth = 1.2) +
    ggplot2::annotate("text", x = K_TRUE, y = hkpca$h90, label =
sprintf(" r \n (%d)", K_TRUE), hjust = -0.1, color = "forestgreen",
fontface = "bold", size = ref_size) +
    ggplot2::geom_vline(xintercept = K_rlzd, color = "purple",
linetype = "dotdash", linewidth = 1) +
    ggplot2::annotate("text", x = K_rlzd, y = hkpca$h75, label =
sprintf(" K_rlzd \n (%d)", K_rlzd), hjust = 1.05, color = "purple",
fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = kpca_kaiser, color =
"darkorange", linetype = "dashed", linewidth = 1) +
    ggplot2::annotate("text", x = kpca_kaiser, y = hkpca$h52, label =
sprintf("Kaiser, E>Mean \n(%d) ", kpca_kaiser), hjust = 1.05, color =
"darkorange", fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = (n_rows - 1), color = "black",
linetype = "dotted", linewidth = 1) +
    ggplot2::annotate("text", x = (n_rows - 1), y = hkpca$h30, label =
"Rank \nCeiling (N-1)", hjust = 1.05, size = ref_size) +
    ggplot2::scale_x_log10(breaks = log10_breaks) +
    ggplot2::coord_cartesian(ylim = c(kpca_y_min, kpca_y_max)) +
    ggplot2::labs(title = "Kernel PCA (RBF)", x = "Eigenvalue Index
[Log Scale]", y = "Eigenvalue") +
    ggplot2::theme_minimal(base_size = 12) + ggplot2::theme(plot.title
= ggplot2::element_text(face="bold"))
  ...
  p_ent <- ggplot2::ggplot(ent_data, ggplot2::aes(x = Rank, y = Eigenvalue)) +
    ...
    ggplot2::geom_vline(xintercept = K_TRUE, color = "forestgreen",
linetype = "solid", linewidth = 1.2) +
    ggplot2::annotate("text", x = K_TRUE, y = hent$h90, label =
sprintf(" r \n (%d)", K_TRUE), hjust = -0.1, color = "forestgreen",
fontface = "bold", size = ref_size) +
    ggplot2::geom_vline(xintercept = K_rlzd, color = "purple",
linetype = "dotdash", linewidth = 1) +
    ggplot2::annotate("text", x = K_rlzd, y = hent$h75, label =
sprintf(" K_rlzd \n (%d)", K_rlzd), hjust = 1.05, color = "purple",
fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = ent_kaiser, color = "darkorange",
linetype = "dashed", linewidth = 1) +
    ggplot2::annotate("text", x = ent_kaiser, y = hent$h52, label =
sprintf("Kaiser, E>Mean \n(%d) ", ent_kaiser), hjust = 1.05, color =
"darkorange", fontface = "italic", size = ref_size) +
    ggplot2::geom_vline(xintercept = (M_PROXIES - 1), color = "black",
linetype = "dotted", linewidth = 1) +
    ggplot2::annotate("text", x = (M_PROXIES - 1), y = hent$h30, label
= "Rank \nCeiling (m-1)", hjust = 1.05, size = ref_size) +
    ggplot2::scale_x_log10(breaks = log10_breaks) +
    ggplot2::coord_cartesian(ylim = c(ent_y_min, ent_y_max)) +
    ggplot2::labs(title = "Entropic Scree", x = "Eigenvalue Index [Log
Scale]", y = "Eigenvalue") +
    ggplot2::theme_minimal(base_size = 12) + ggplot2::theme(plot.title
= ggplot2::element_text(face="bold"))
  ...
  if (requireNamespace("patchwork", quietly = TRUE)) {
    ...
    cat("\n[!] To view the side-by-side 4-panel plot, please install
...
# ==============================================================================
# Helper to find the naive empirical elbow for baselines (Max Log-Gap,
skipping K=1)
get_naive_elbow <- function(eig_vals, max_rank) {
  ...
  # Skip the first gap (index 1 to 2) to force K >= 2, mirroring
Entropic Scree logic
  if (length(gaps) >= 2) {
...
format_elbow_result <- function(elbow_val, k_true, k_rlzd) {
  ...
  if (abs(elbow_val - k_rlzd) <= max(2, k_rlzd * 0.1))
return(sprintf("%d (~K_rlzd)", elbow_val))
