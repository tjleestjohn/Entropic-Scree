...
cont_cols <- names(observed_data)[sapply(observed_data, function(x)
...
if(length(cont_cols) > 0) {
  ...
  bin_extreme <- ceiling(n_rows^(2/3)) # Safe fallback if no
continuous vars exist
}
...
create_panel <- function(eig_vec, title_text, hide_y_label = FALSE) {
  ...
  p <- ggplot(df, aes(x = Rank, y = Eigenvalue)) +
    ...
    # True generative boundary (Dynamically points to K_TRUE from
global environment)
    geom_vline(xintercept = K_TRUE, color = "#D55E00", linetype =
"dashed", linewidth = 1.2) +
    scale_y_continuous(trans = 'log10') +
    # Use coord_cartesian to visually zoom without discarding the
first data point
...
# Generate the 3 panels dynamically (bquote and sprintf insert the
exact dynamic bin counts)
p1 <- create_panel(eig_default, bquote(atop(bold("Conservative
Default" ~ (N^{1/3})), bold(.(sprintf("[%d Bins]", bin_default))))),
hide_y_label = FALSE)
p2 <- create_panel(eig_dense, bquote(atop(bold("Dense" ~ (N^{1/2})),
bold(.(sprintf("[%d Bins]", bin_dense))))), hide_y_label = TRUE)
p3 <- create_panel(eig_extreme, bquote(atop(bold("Freedman-Diaconis" ~
phantom(N^{1/2})), bold(.(sprintf("[%d Bins]", bin_extreme))))),
hide_y_label = TRUE)
